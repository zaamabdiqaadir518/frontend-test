...
import SearchItem10 from "../../components/searchItem/searchItem10";

const SearchItems = () => {

  const [hotel, SetHotel]= useState([])
  const handleReadData = ()=>{
      axios.get("http://localhost:8800/Hotels").then((response)=>{
  ...
  if(key){
    axios.get(`http://localhost:8800/hotel/search/${key}`).then((response) =>{
        SetHotel(response.data)
    }).catch((error)=>{
        console.log(error)
    })
